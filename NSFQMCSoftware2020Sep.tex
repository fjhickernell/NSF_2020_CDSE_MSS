...
\end{enumerate}
Here we provide an overview of QMC, a description of our early QMCPy efforts, our plans to grow QMCPy, the benefits of QMCPy to the larger community, and why our team is best equipped to carry out this effort.
...
\end{equation} 
cannot be computed analytically.  But, it may be estimated the sample mean,
...
To construct good points that mimic distributions other than standard uniform, one must make a transformation, often the inverse cumulative distribution.  The \pyinline{TrueMeasure} class takes care of that transformation automatically.
\begin{pythoncode}
...
The first way constructs the Gaussian transformed LD \pyinline{TrueMeasure} instance \pyinline{gs} as discussed in the previous section.  One defines the function $g$ as in \eqref{eq:KeisterAlt} above, and then uses \pyinline{CustomFun} to construct an integrand $f$ such that the integral can be written in terms of the uniform distribution, which the LD sequence mimics.  The statement below involving \pyinline{CustomFun} constructs the transformation $\bT$ in \eqref{eq:KeisterAlt}.
\begin{pythoncode}
...
gs = qmcpy.Gaussian(qmcpy.Sobol(d), covariance = 1/2)    #choose the Gaussian distribution
...
The final stage of the computation requires the construction of a \pyinline{StoppingCriterion} instance, \pyinline{sc}.  This one is from \cite{HicJim16a}, and his based on Walsh transformations of the sampled integrand data.  Invoking the \pyinline{integrate} method \pyinline{sc} computes $\hmu_n$ satisfying
\begin{equation} \label{eq:error_crit}
	\abs{\mu -\hmu_n} \le \varepsilon
\end{equation}
where $\varepsilon$ is the absolute error tolerance.

Another way to compute the Keister integral, \eqref{eq:Keister}, is to think of it as an integral with respect to the Lebesgue mesaure.  Again \pyinline{CustomFun} makes the proper variable transformation, and the answer is the same as the first way.  This second way takes about $0.45$ s, whereas the first way takes about $0.25$ s.
