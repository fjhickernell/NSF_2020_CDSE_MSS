...
\subsubsection{\textup{\pyinline{DiscreteDistribution}}} LD sequences are constructed by creating the object and then generating the points.  The code below illustrates how to generate the points shown in the center panel of Fig.\ \ref{fig:iid_vs_ld}.
\begin{pythoncode}
...
\subsubsection{\textup{\pyinline{TrueMeasure}}} 
...
The first way constructs the Gaussian transformed LD \pyinline{TrueMeasure} instance \pyinline{gs} as discussed in the previous section.  One defines the function $g$ as in \eqref{eq:KeisterAlt} above, and then uses \pyinline{CustomFun} to construct an integrand $f$ such that the integral can be written in terms of the uniform distribution, which the LD sequence mimics, i.e., $\mu = \int_{[0,1]^d} f(\bx) \, \dif \bx$.  The statement below involving \pyinline{CustomFun} makes that transformation.
\begin{pythoncode}
