...
Although QMCPy already includes the most popular LD sequences, there is lack of flexibility, and some more recent sequences are missing.

Presently, the digital shifting and scrambling  recommended for Sobol' sequences must be built in by the creator of the \pyinline{backend}, namely, PyTorch \cite{PyTorch}, MPS \cite{Nuy17a}, and qrng \cite{QRNG2020}  However, the generating matrices (direction vectors) for the Sobol' sequence should be able to be specified independently of the algorithm the part of the algorithm that digitally scrambles and shifts.  We aim to rectify this, which will allow folks to try the BRODA generators \cite{BRODA20a} with scrambling, which is not yet possible.

Sobol' sequences are a member of the digital sequence family.  LatNet Builder \cite{LatNet} can construct the generating vectors for polynomial lattices,  another kind of digital sequence.  When our Sobol' sequence generator allows the the generating matrices to be specified independently of the digital scrambling and shifting, we will be able to allow users to explore whether 
