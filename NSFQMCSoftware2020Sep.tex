...
def h(t):  #your desired integrand, calculations must be vectorized
 ...
 \begin{itemize}
 	...
 	\item \pyinline{SoppingCriteria} based on the Central Limit Theorem (CLT) for IID sampling and random replications of LD sampling;  a more rigorous stopping criterion for IID sampling based on a Berry-Essen inequality \cite{HicEtal14a};  stopping criteria based on Fast Fourier/Walsh Transforms via tracking the decay of the series coefficients \cite{HicJim16a, JimHic16a} and using a Bayesian perspective \cite{RatHic19a};
...
The overall vision is that QMCPy aggregates the best QMC software with a clear structure and a consistent user interface.  When a potentially better LD generator, algorithm, or use case comes arises, it should be a simple matter to swap out the new for the old and measure the performance improvement.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Feature Rich LD Sequence Generators}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Although QMCPy already includes the most popular LD sequences, there is lack of flexibility in their implementation.  Some more recent sequences are missing.

At present, the digital shifting and scrambling  recommended for Sobol' sequences must be hard coded by the creator of the \pyinline{backend}, namely, PyTorch \cite{PyTorch}, MPS \cite{Nuy17a}, and qrng \cite{QRNG2020}.  However, for flexibility, the generating matrices (direction vectors) for the Sobol' sequence should be specified independently of the part of the generator that digitally scrambles and shifts.  \emph{We will rectify this}, which will allow folks to try scrambled BRODA generators \cite{BRODA20a}.  This is not yet possible.

Sobol' sequences are a member of the digital sequence family \cite{DicPil10a}.  LatNet Builder \cite{LatNet} can construct the generating matrices for polynomial lattices,  another kind of digital sequence.  When our Sobol' sequence generator allows the the generating matrices to be specified independently of the digital scrambling and shifting, users can explore  \emph{what advantages polynomial lattices have} over Sobol' sequences.

For integrands with higher orders smoothness, higher order digital sequences \cite{Dic09a, Dic11a} can provide faster convergence rates as $n \to \infty$, which translate into slower computational cost rates as  $\varepsilon \to 0$.  Although some QMC applications, such as those in quantitative finance, often have integrands with low order smoothness, other applications, such as computing probabilities over subsets of the sample space  involving smooth densities, have higer order smoothness.  \emph{Higher order digital sequences will be implemented} in QMCPy.

One large family of discrepancies are defined in terms of a symmetric, positive definite kernel, $K$.  If we identify $K(\bt,\bx)$ as some inner product of point distributions, i.e., $K(\bt,\bx) = \ip[\calM]{F_{\{\bt\}}}{F_{\{\bx\}}}$,  then the discrepancy between two distributions can be defined as the norm of the distance between the two \cite{Hic99a}:
...
\end{multline}
For error bound \eqref{eq:KH}, $K$ plays the role of a reproducing kernel for the Hilbert space, $\calf$.  There is a Bayesian version of \eqref{eq:KH} for integrands with covariance kernel $K$ \cite{Hic17a}.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Stronger Algorithms}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
