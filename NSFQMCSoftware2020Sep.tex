...
Quasi-Monte Carlo (QMC)methods replace independent and identically distributed (IID) points by low discrepancy (LD) points to achieve substantial efficiency gains.  We propose to grow a Python QMC software library that will be shared by QMC researchers and benefit an expanding base of QMC users.

\subsection{Applications of (Q)MC}  Many situations are best understood by building randomness into the mathematical model.  Examples include statistical inference, financial risk, light transport, power grid resilience, and particle physics.  Simulations of these situations using random vectors or discrete stochastic processes, allows one to estimate expectations (means, averages), quantiles, and other statistical properties of the outcomes.  Multi-dimensional integrals can be written as  expectations.  (Q)MC methods avoid the curse of dimensionality that would arise from gridded samples.
...
The quantity of interest, $Y$ (option payoff, pixel intensity, time to failure), is typically expressed as a function of a $d$-dimensional vector random variable $\bX$, i.e., $Y = f(\bX)$.  When $f$ is complex, the population mean, $\mu = \bbE(Y)$,  may be estimated the sample mean,
\begin{equation}
\hmu_n = \frac 1n \sum_{i=1}^n Y_i = \frac 1n  \sum_{i=1}^n f(\bX_i).
\end{equation}
How does one choose $\bX_1, \bX_2, \ldots $ to make $\abs{\mu - \hmu_n}$ small?
...
$\calu[0,1]^2$.  The points on the left, $\bX_1, \bX_2,  \ldots \IIDSim \calu[0,1]^2$, do not know about each other.  They are 
random.  Since they are IID, the multivariate distribution of $\bX_1, \ldots \bX_n$ is  $F_{n}(\bx_1, \ldots, \bx_n) = F(\bx_1) 
\cdots F(\bx_n) = x_{11}x_{12} x_{21} \cdots x_{n2}$, where $F(\bx)  = x_1x_2$ in this case.



\begin{figure}
	\centering
	\includegraphics[height = 6cm]{ProgramsImages/iid_uniform_pts.png} \quad
	\includegraphics[height = 6cm]{ProgramsImages/lattice_pts.png}
	\caption{IID points (left) and LD lattice points (right).  The LD points have fewer gaps and clusters of points. \label{fig:iid_vs_ld}}
\end{figure}

In contrast, the  points on the right, $\bX_1, \bX_2,  \ldots \LDSim \calu[0,1]^2$, are coordinated with each other.  They might be deterministic or random.  They are designed to mimic the distribution $F$ will in the sense that the empirical distribution function of  $\bX_1, \ldots \bX_n$, which is denoted, $F_{\{\bX_i\}_{i=1}^n}$ should be close to $F$.  (The empirical distribution for $n$  points assigns probability $1/n$ to each of point.)  A \emph{discrepancy}, $D(\{\bX_i\}_{i=1}^n, F)$, measures the size of $F - F_{\{\bX_i\}_{i=1}^n}$, and LD points make the discrepancy small.
