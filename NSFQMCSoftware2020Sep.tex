...
\iffalse
Cubatures (approximation of multivariate integrals) based on grid sampling have computational costs that increase exponentially with the number of variables, $d$.  (Q)MC cubature avoids this curse of dimensionality with computational costs that are often only weakly dimension dependent.
...
So far we have discussed the estimation of the integral $\mathbb{E}[f(\bX)] = \int f(\bx) dF(\bx)$ using samples on distribution $F$. However, if we knew that the integrand $f$ is nearly zero outside an importance region $A$ where $\mathbb{P}(\bX \in A)$ is small, both IID or LD sampling on $F$ would place few points within region $A$. The sample mean estimate will then be highly \textit{variable} from the population mean, which is undesirable. Such problems are common in rare event modeling \cite{rubino2009rare}, Bayesian inference \cite{salmeron2000importance}, amont other applications.

Importance sampling (see, e.g., \cite{owen2000safe}) is a powerful method for reducing this estimation variance. The idea is to use samples from a new distribution (the \textit{importance} distribution) which places greater weight on the importance region $A$. Let $Q$ be such a distribution on the same sample space $\Omega$. Then the desired integral can be written as:
\begin{equation}\label{eq:imp}
\mathbb{E}[f(\bX)] = \int f(\bx) dF(\bx) = \int \frac{f(\bx)dF(\bx)}{dQ(\bx)} dQ(\bx) =: \int \tilde{f}(\bx) dQ(\bx)
\end{equation}
To estimate $\mathbb{E}[f(\bX)]$, importance sampling uses samples from $Q$ with the modified integrand $\tilde{f}$. The challenge lies in finding a good choice for $Q$, which varies greatly from problem to problem. Important developments include (among many) the annealed importance sampler \cite{neal2001annealed}, the bridge sampler \cite{gelman1998simulating}, and the safe importance sampler \cite{owen2000safe} by \AO. Recent works include \cite{mueller2019neural}, which use deep neural networks for importance sampling in image rendering, and \cite{huling2020energy}, where the PI \SM extends importance sampling for covariate balancing in causal inference.

Control variates \cite{glasserman2013monte} is another powerful variance reduction technique. The idea is to use information on errors in estimates of a \textit{known} integral (the control variate), to improve the estimate of an \textit{unknown} integral. When these two integral estimates are correlated, the control variates estimator incorporates this to reduce the variance of the desired integral estimate. Control variates are widely used in financial pricing and stochastic simulations. Important works (among many) include \cite{nelson1990control}, which investigates control variate remedies, and the control variates for QMC paper \cite{hickernell2005control} by \AO and the PI \FH. Recently, \cite{mueller20neural} introduced the neural control variates method, which uses a neural network to learn an optimal choice of control variate for variance reduction.

\textit{We will implement these existing variance reduction methods} into QMCPy. This will provide users with a comprehensive and accessible toolbox of variance reduction methods for a broad range of applications, from financial engineering to computer graphics.
...
Much of the literature on QMC focuses on the estimation of the population \textit{mean}. In many problems, however, one may be interested in estimating not only the mean from samples, but also its \textit{density}. This is the problem of \textit{density estimation}. Density estimation was classically studied in statistics under the setting where, given IID samples $\bX_1, \cdots, \bX_n$ from distribution $F$, one wishes to estimate the density of $F$. The state-of-the-art method is kernel density estimation \cite{silverman1986density}, which approximates the density of $F$ as:
\begin{equation}\label{eq:kerndens}
\hat{p}_n(\bx) = \frac{1}{n} \sum_{i=1}^n \frac{1}{h}k\left( \frac{\bx - \bX_i}{h} \right),
\end{equation}
where $k$ is a symmetric kernel, and $h>0$ is the bandwidth parameter. \SMNote{should we instead use $g$ for integrands, and $f$ for densities?}

Given the improvement that QMC gives for mean estimation, a natural question is whether QMC also provides gains for density estimation. A recent paper \cite{abdellah2018density} by \AO and coauthors explores this by replacing the IID samples $\{\bX_i\}_{i=1}^n$ in \eqref{eq:kerndens} with the LD samples $\{F^{-1}(\mathbf{U}_i)\}_{i=1}^n$, where $\{\mathbf{U}_i\}_{i=1}^n$ are randomized LD points \cite{owen2000monte} on the uniform distribution $F_{\unif}$. The authors showed that this new QMC density estimator enjoys an improved rate of convergence over the standard estimator \eqref{eq:kerndens}, both theoretically and in numerical experiments. 
...
A recent application of QMC is to compute the expectation of a functional of solution of a partial differential equation with random coefficients \cite{HerSch20a}.  An example is Darcy's flow through rock with random porosity.  QMCPy will implement a typical use case, which may show how to link QMCPy code with a PDE solver from another package. \SMNote{I will add a bit more on this if there's room.}
...
\SM is a statistician who became quite familiar with QMC through the aforementioned SAMSI program. He is a tenure track assistant professor of statistics, and his interests are in Bayesian modeling, big data analytics, and computer experiments. \SM provides expertise on statistical methodology and applications: he is an Associate Editor for \textit{Technometrics} (the top journal for engineering statistics), the recipient of the Statistics in Physical Engineering Science Award from American Statistical Association (ASA), and multiple student paper awards from the ASA and the Institute for Operations Research and the Management Sciences (INFORMS). \SM will lead the activities at Duke and oversee the big data learning and Bayesian inference efforts.
