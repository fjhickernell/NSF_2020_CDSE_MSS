...
Importance sampling (see, e.g., \cite{owen2000safe}) samples from a new distribution (the \textit{importance} distribution) to place greater weight where the integrand varies more. Since LD sampling at its heart is $\calu[0,1]^d$, LD sampling to mimic a different distribution can be thought of as choosing a variable transformation. We describe it that way.  The desired integral originally defined in terms of $g$ is
