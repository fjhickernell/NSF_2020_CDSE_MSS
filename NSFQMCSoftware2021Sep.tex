...
One approach for LD posterior sampling is to minimize the kernel discrepancy $D(\{\bX_i\}_{i=1}^n, F)$ in \eqref{eq:kerdisc} for samples $\{\bX_i\}_{i=1}^n$. This is known as \textit{kernel herding} \citep{chen2012super}. Kernel herding has a key limitation: the discrepancy, $D(\{\bX_i\}_{i=1}^n, F)$, requires an analytic form for the integral $\int K(t,\bX_i) \, \dif F(t)$, which is unattainable for complex posteriors $F$. To address this, \cite{chen2018stein} proposes the \textit{Stein reproducing kernel}:
...
\end{multline}
where $K$ is a symmetric, positive definite kernel, and $\nabla$ and $\nabla \cdot$ are the gradient and divergence operators. The key advantage is that the integral in question, $\int K_{st}(t,\bX) \, \dif F(t)$, evaluates to 0 for all $\bX$, thereby allowing one to generate LD posterior samples via direct minimization of discrepancy $D(\{\bX_i\}_{i=1}^n, F)$. Stein points then start with the mode, and generate sequence of samples which approximate the posterior. This allows for computational savings for the learning complex Bayesian models.
