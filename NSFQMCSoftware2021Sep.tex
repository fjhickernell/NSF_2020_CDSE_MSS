...
\begin{enumerate}[i.]
    ...
    \begin{enumerate}[a)]
        \item A variety of high performance LD sequence generators (\cref{sec:richLD}),
        \item QMC algorithms, including automatic stopping criteria, adaptive variance/variation reduction (\cref{sec:varred}), multilevel algorithms (\cref{sec:multilevel}), big data analytics (\cref{sec:bigdata}), and Bayesian computation (\cref{sec:bayes}), and
        \item Illuminating use cases in timely applications, including machine learning, uncertainty quantification, Bayesian methodology, quantitative finance, and other fields (\cref{sec:scopeapplication});
    \end{enumerate}
...
\begin{enumerate}[i.]
\item Attaining $\Order(n^{-3/2})$ convergence with scrambled nets for integrals over $\reals^d$;
\item Stopping criteria for higher-order nets and for multilevel QMC cubature;
\item QMC methods for multilevel function approximation;
\item LD subsampling methods for big data analytics; and
\item LD posterior sampling methods for expensive Bayesian modeling problems.
\end{enumerate}
...
\hypertarget{TSlink}{Tim Sullivan} (\TS, Warwick University), \hypertarget{ASlink}{Aleksei Sorokin } (\AS, PhD student, Illinois Tech), \hypertarget{IJlink}{Irene Ji} (\IJi, PhD student, Duke University), \hypertarget{TTlink}{Tao Tang} (\TT, PhD student, Duke University), as well as other students, alumni, and friends. 
...
$\std(Y)/\sqrt{n}$, where $\std(Y) = \sqrt{\int_{\cube} \abs{f(\bx) - \mu}^2 \, \dif \bx}$.  The smoothness required of $f$ is minimal, and the error bound has no ``curse of dimensionality'' assuming $\std(Y)$ does not explode with increasing dimension.  However, the convergence rate of $\Order(n^{-1/2})$ is modest, translating into a computational cost of $\Order(\varepsilon^{-2})$ to satisfy error criterion \eqref{eq:error_crit}.
...
\cref{fig:KeisterTimes} displays results from QMCPy \cite{QMCPy2020a} for  an example of Keister \cite{Kei96} for $d =5$ and $\mu \approx 1.135$:
...
The \pyinline{TrueMeasure} class automates the transformation required to construct good points that mimic distributions other than standard uniform.  The  inverse  distribution is often used.  \cref{fig:ld_Gauss} displays  IID and Sobol' points  transformed to mimic the Gaussian distribution with mean $\begin{pmatrix} 3 \\ 2 \end{pmatrix}$ and covariance matrix $\begin{pmatrix} 9 & 5 \\ 5 & 4 \end{pmatrix}$.  
\begin{pythoncode}
...
\subsection{Multilevel QMC (MLQMC)} \label{sec:multilevel} [\SM lead, \FH, \IJi, \TT{}] 
...
\begin{theorem}
Let $\varepsilon > 0$ be a desired error tolerance. Assume the bias of $f_l$ is bounded as $\sup_{\bx \in [0,1]^d} |f_l(\bx)-f(\bx)| = \mathcal{O}(T_1^{-\alpha l})$ for some $T_1 > 0$. Further assume $C_l$, the cost of a single evaluation of $f_l$, is bounded as $C_l = \mathcal{O}(T_2^{\beta l})$ for some $T_2 > 0$. Suppose the points $\{\bX_i^l\}_{i=1}^{n_l}$ are taken from a low-dispersion QMC sequence (see \cite{niederreiter1992random,yakowitz2000global}). Then, given a total cost budget of $C_\varepsilon$ and under mild regularity conditions on $f$, there exists a number of levels $L$ and sample sizes $n_1, \ldots, n_L$ such that $\sup_{\bx \in [0,1]^d} |f(\bx) - \hat{f}_L(\bx)| < \varepsilon$ with total cost $C_\varepsilon = \mathcal{O}(\varepsilon^{-d/\nu})$ if $\alpha d > 2 \beta \nu$.
\label{thm:mlqmc}
\end{theorem}
\noindent This theorem shows that, when the bias of the lower-fidelity functions $\{f_l\}_{l=1}^L$ and its evaluation costs are sufficiently bounded, a QMC sampling of design points (optimally allocated over each level) yields the desired error tolerance $\varepsilon$ with total cost bounded by $C_\varepsilon = \mathcal{O}(\varepsilon^{-d/\nu})$. Under similar conditions, this rate on $C_\varepsilon$ can be shown to be faster than existing complexity rates using single-level predictors or Monte Carlo sampling \citep{wendland2004scattered}. This shows that QMC sampling is indeed effective at reducing cost complexity at a fixed error level $\varepsilon$, or equivalently, reducing approximation error given fixed total costs. This cost reduction is crucial in expensive approximation problems in physical science and engineering applications (see below).

\vspace{0.2cm}
\noindent \textit{Adaptive Sampling}. While \cref{thm:mlqmc} guarantees an optimal $L$ and sample sizes $n_1, \ldots,n_L$ achieving the desired cost complexity $C_\varepsilon = \mathcal{O}(\varepsilon^{-d/\nu})$, these depend on parameters of the underlying GP model, which are unknown in practice. This optimal allocation of points needs to be determined \textit{adaptively} via sequential sampling of the lower-fidelity functions $f_l$. We will develop an adaptive strategy for sequentially allocating samples $n_1, \cdots, n_L$ and a stopping rule for determining $L$, which provably achieves the cost complexity guarantee in \cref{thm:mlqmc}.

\vspace{0.2cm}

\noindent \textit{Applications}. We will demonstrate the effectiveness of MLQMC approximation approach for a range of important scientific computing problems. The PI \SM is active in multidisciplinary collaborations for nuclear physics \citep{ji2021graphical,everett2021multisystem,everett2021phenomenological,cao2021determining}, aerospace engineering \citep{mak2018efficient,yeh2018common,chang2019kernel,chang2021reduced}, and material sciences \citep{chen2020function,chen2019adaptive}, where such adaptive sampling would greatly improve predictive models given computational budgets. We will explore the advantages of this methodology for such applications.
...
\subsection{Expensive Bayesian Modeling} [\SM lead, \FH, \TT{}] \label{sec:bayes}
...
\SMNote{cut down a bit} \FH has been the lead PI on the GAIL \cite{ChoEtal21a} MATLAB software project that contains many of the stopping criteria incorporated into QMCPy.  His expertise is in the numerical analysis of QMC and other algorithms for multivariate problems.  He has also developed several theoretically justified adaptive numerical algorithms.  As a former editorial board member for major computational mathematics journals, a Fellow of the Institute of Mathematical Statistics, and a co-leader of SAMSI's program on QMC in 2017-18, \FH understands the interface between computational mathematics and statistics.  \FH will lead the activities at Illinois Tech and oversee QMCPy's architecture.  He will also lead the development of higher-order nets, net scrambling, and multilevel methods.
