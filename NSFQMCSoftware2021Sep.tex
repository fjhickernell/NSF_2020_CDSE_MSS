...
\SCTCNote{Fig 3 and citation for NAG missing}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Start of QMCPy}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The better existing QMC software packages include
...
\subsubsection {Variance/Variation Reduction Through Importance Sampling and Control Variates} [Years 1--3]  The discrepancy in \eqref{eq:KH} measures the difference between the empirical distribution of the sampling sites and the uniform distribution.  In many cases, the original problem is posed as an integral over $\reals^d$, as on the left side of \eqref{eq:imp}, in which case a variable transformation must be performed to obtain an integral over $[0,1]^d$.  Note that $f_{\bT}(\bx) = g(\bT(\bx)) \lambda(\bT(\bx)) \abs{(\bT'(\bx))}$, where $\abs{(\bT'(\bx))}$ denotes the Jacobian.  While one may choose $\bT$ such that $\lambda(\bT(\bx)) / \abs{(\bT'(\bx))} = 1$, this is not the only choice and may not be the best choice.  Choosing the transformation $\bT$ is equivalent to performing importance sampling.   
