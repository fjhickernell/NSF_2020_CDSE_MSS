...
\label{eq:mlpred}
\end{equation}
In what follows, we will use a Gaussian process predictor \cite{santner2003design} with Mat\'ern correlation \cite{stein2012interpolation}, but this can be flexible in practice. Clearly, the placement of evaluation points $\{\bX_i^l\}_{i=1}^{n_l}$, $l=1, \ldots, L$ is paramount for maximizing predictive accuracy: these points should be well spread out over $[0,1]^d$ for each level $l$, and the sample sizes $n_1, \ldots, n_L$ should be chosen to balance predictive accuracy and evaluation cost. We show next that a careful allocation of QMC points over each level allows for provably improved predictions over the state-of-the-art. Related work on multi-level approximation \cite{kennedy2000predicting,haaland2011accurate,le2014recursive,le2015cokriging} deal largely with the modeling of the multi-level predictor, but not the design of the multi-level samples, which we investigate here.

\SMNote{The following theorem shows that ... . leverages theory from giles. quick discussion.
...
\subsubsection{Motivation and Preliminary Results} Big data is ubiquitous with recent advances in experimental technology and computing power. A key challenge is that learning algorithms need to be highly \textit{efficient} and \textit{scalable} to extract information for real-time decision-making. One way to make these algorithms scalable is to iteratively train the model on small batches of the data, typically sampled uniformly at random. This is used to scale up many powerful state-of-the-art machine learning algorithms, such as stochastic gradient descent (SGD, \cite{Bot2010}) and stochastic gradient boosting \cite{friedman2002stochastic}.
...
\begin{equation}
K_{\rm data}(\bx,\by) = \sum_{\bk \in \mathbb{Z}^d \setminus \mathbf{0}} \lambda_{\bk} \phi_{\bk}(\bx)\overline{\phi_{\bk}(\by)}, \quad \phi_{\bk}(\bx) = e^{2 \pi i \bk^T\bx} - b_{\bk}, \quad b_{\bk} = \frac{1}{N} \sum_{m=1}^N e^{2 \pi i \bk^T\bX_m}.
\end{equation}
Here, the coefficients $b_{\bk}$ can be efficiently computed via non-linear fast Fourier transform \citep{wahls2015fast}. This data kernel $K_{\rm data}$ has two appealing properties. We can show that the subsample $\mathcal{X}_s$ that minimizes the kernel discrepancy with $K_{\rm data}$ indeed yields the improved optimization rate in Theorem \ref{thm:ldsgd}. We can also show that, with all coefficients computed, this kernel discrepancy can be evaluated in $\mathcal{O}(n^2)$ work, \textit{independent} of $N$ (the big data sample size). We will develop efficient and provably correct algorithms for optimizing this data kernel discrepancy, leveraging recent developments in accelerated gradient descent \citep{jin2018accelerated} and randomized algorithms \citep{mahoney2011randomized}.
...
We will tackle the following tasks to develop RSPs as an efficient LD Bayesian inference within QMCPy:
\begin{itemize}
\item \textit{Task 3.6.1 - Theory}: \SMNote{a few sentences: marginal convergence of samples, CLT for CIs.}
\item \textit{Task 3.6.2 - Application}: We will also demonstrate the effectiveness of RSPs in a suite of practical Bayesian modeling problems where the posterior is expensive. PI \SM has worked on a range of such problems, including climatology \cite{mak2016regional}, aerospace engineering \cite{mak2018efficient,chang2019kernel,yeh2018common,chang2021reduced}, and nuclear physics \cite{ji2021graphical}, and we will explore the effectiveness of RSPs for such problems. 
\end{itemize}
