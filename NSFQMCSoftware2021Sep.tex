...
\hypertarget{RZlink}{Ruda Zhang} (\RZ, post-doc, Duke University),
...
\begin{equation}
\hat{f}_L(\bx) = \sum_{l=1}^L \mathcal{P}_l(\bx), \;\; \text{where $\mathcal{P}_l(\bx)$ is a predictor for \textit{refinement} $f_l(\bx) - f_{l-1}(\bx)$, $l = 1, \ldots, L$.}
\label{eq:mlpred}
\end{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
While the existence of a LD subset is promising, one challenge is in finding such a subset efficiently. We will find this via the following optimization approach. Define the so-called ``data kernel'', defined using the big data $\mathcal{X} = \{\bX_m\}_{m=1}^N$:
\begin{equation}
...
We have a well-constructed  team that combines senior personnel with diverse backgrounds, career stages, and institutions.  We will grow QMCPy into what it should become while providing new theories and methodologies to underpin our new algorithms. The leads and team members for each topic are identified above.  The senior personnel---together with our students and collaborators---will have regular meetings to share our progress and brainstorm next steps. These will be held both at our own institutions and via video conference among institutions.
...
\subsection{Students}  \AS began his applied mathematics PhD studies at Illinois Tech in Fall 2021 after competing a dual BS/MS degree at Illinois Tech and doing a large majority of the coding of QMCPy  to date. \IJi and \TT are 3rd year Statistical Science PhD students at Duke University, and work on theory and methods in computer experiments, uncertainty quantification, Bayesian learning and QMC, with applications to heavy-ion and nuclear physics.
