...
\item Stopping criteria for higher-order digital net and multilevel QMC cubatures (\cref{sec:threehalves});
\item Multilevel QMC function approximation;
\item LD subsampling methods for big data analytics to decrease computational cost; and
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Performance} [\FH lead, \SCTC, \YD, \AS, \DN, \AO{}] \label{sec:performance}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[Years 2--3] Josef Dick introduced the concept of higher order nets, and showed how these could give $\Order(n^{-\alpha})$ convergence rates for QMC cubature, where $\alpha$ is limited by the smoothness of the integrand and the order of the net \cite{Dic08a,Dic09a,Dic11a}.  The stopping criteria developed by PI \FH and his collaborators \cite{HicJim16a,HicEtal17a,Jag19a}, are not designed for these higher order nets because they only track the decay of the Walsh coefficients of the integrand. We will \emph{develop stopping criteria for higher order net cubature}.  We will extend by extending \cite{HicJim16a} by tracking a more nuanced measure of the fast Walsh coefficients of the integrand that matches the definition of higher order nets.  We will extend the Bayesian stopping criterion \cite{Jag19a} by identifying smoother digital shift invariant covariance kernels whose Gram matrices can be decomposed by a fast Walsh transform.
