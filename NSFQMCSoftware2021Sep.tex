...
\begin{itemize}
\item \textit{Task 3.5.1}: While the existence of a LD subset is promising, one challenge lies in finding such subset efficiently. We will find this subset the following optimization approach. Define the so-called ``data kernel'', defined using the big data $\mathcal{X} = \{\bX_m\}_{m=1}^N$:
\begin{equation}
...
\item \textit{Task 3.5.2}: We will provide an efficient and well-documented implementation of LD-batch SGD within QMCPy. Given recent breakthroughs in high-performance and distributed computing architectures, we will develop scalable algorithms which exploit such technologies for efficient LD-batch subsampling. We will also provide specific implementations of LD-batch SGD for popular learning models (e.g., regression, neural nets, Gaussian processes) on QMCPy.
\end{itemize}   
...
\end{multline}
where $K$ is a symmetric, positive definite kernel, and $\nabla$ and $\nabla \cdot$ are the gradient and divergence operators. The key advantage is that the integral in question, $\int K_{st}(t,\bX) \, \dif F(t)$, evaluates to 0 for all $\bX$, thereby allowing one to generate LD posterior samples via direct discrepancy minimization. With this closed-form $D(\{\bX_i\}_{i=1}^n, F)$, \cite{chen2018stein} makes use of a sequential optimization procedure: the first sample $\bX_1$ is taken at the mode of $F$, then subsequent samples $\bX_2, \bX_3, \cdots$ are obtained by a sequential optimization of $D(\{\bX_i\}_{i=1}^n, F)$. The optimized samples $\{\bX_i\}_{i=1}^n$ This allows for computational savings for the learning complex Bayesian models.

Despite its promise, Stein points have an important limitation: they do not provide inference for computed posterior quantities.
