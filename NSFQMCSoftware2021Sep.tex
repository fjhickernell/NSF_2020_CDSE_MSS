...
\begin{itemize}
\item \textit{Task 3.5.1 - Optimization}: While the existence of a LD subset is promising, one challenge lies in finding such subset efficiently. We will find this subset the following optimization approach. Define the so-called ``data kernel'', defined using the big data $\mathcal{X} = \{\bX_m\}_{m=1}^N$:
\begin{equation}
K_{\rm data}(\bx,\by) = \sum_{\mathbf{k} \in \mathbb{Z}^d \setminus \mathbf{0}} \lambda_{\mathbf{k}} \phi_{\mathbf{k}}(\bx)\overline{\phi_{\mathbf{k}}(\by)}, \quad \phi_{\mathbf{k}}(\bx) = e^{2 \pi i \mathbf{k}^T\bx} - b_{\mathbf{k}}, \quad b_{\mathbf{k}} = \frac{1}{N} \sum_{m=1}^N e^{2 \pi i \mathbf{k}^T\bx_m}.
\end{equation}
Here, the coefficients $b_{\mathbf{k}}$ can be efficiently computed via non-linear fast Fourier transform \citep{wahls2015fast}. This data kernel $K_{\rm data}$ has two appealing properties. We can show that the subsample $\mathcal{X}_s$ that minimizes the kernel discrepancy with $K_{\rm data}$ indeed yields the improved optimization rate in Theorem \ref{thm:ldsgd}. We can also show that, with all coefficients computed, this kernel discrepancy can be evaluated in $\mathcal{O}(n^2)$ work, \textit{independent} of $N$ (the big data sample size). We will develop efficient and provably correct algorithms for optimizing this data kernel discrepancy, leveraging recent developments in accelerated gradient descent \citep{jin2018accelerated} and randomized algorithms \citep{mahoney2011randomized}.

\item \textit{Task 3.5.2 - Implementation}: We will provide an efficient and well-documented implementation of LD-batch SGD within QMCPy. Given recent breakthroughs in high-performance and distributed computing, we will develop scalable algorithms which exploit such technologies for efficient LD-batch subsampling. We will also provide specific implementations of LD-batch SGD for popular learning models (e.g., regression, neural nets, Gaussian processes) on QMCPy.
\end{itemize}   
...
One approach for LD posterior sampling is to minimize the kernel discrepancy $D(\{\bX_i\}_{i=1}^n, F)$ in \eqref{eq:kerdisc} for samples $\{\bX_i\}_{i=1}^n$. This is known as \textit{kernel herding} \citep{chen2012super}. Kernel herding has a key limitation: the discrepancy, $D(\{\bX_i\}_{i=1}^n, F)$, requires an analytic form for the integral $\int K(t,\bX_i) \, \dif F(t)$, which is unattainable for complex posteriors $F$. To address this, \cite{chen2018stein} proposes the \textit{Steinized kernel}:
...
\begin{enumerate}[(i)]
\item \textit{Uncertainty Quantification (UQ)}: UQ is the science of quantifying, characterizing, tracing, and managing uncertainty in computational and physical systems \cite{smartuq, Smi14a}. Since data from such systems are typically expensive to collect, a key focus in UQ is the sampling design for data collection, which can greatly benefit from QMC methods. A recent application of QMC for UQ is to compute the expectation of a functional of solution of a partial differential equation with random coefficients \cite{HerSch20a}. This has broad applications in engineering problems, e.g., fluid flow through a porous medium. QMCPy will implement a typical use case, which may show how to link QMCPy code with a PDE solver from another package. The PI \SM has done extensive work on uncertainty quantification, for rocket engine design \cite{li2017two,li2018uncertainty,chang2019kernel,yeh2018common,mak2018efficient}, neuroscience modeling \cite{wang2020uncertainty}, and active learning \cite{mak2018maximum}, we will explore the performance of LD sampling for these applications.
\item \textit{Bayesian Computation}: \SMNote{need? fill in a few sentences.}
