...
$\std(Y)/\sqrt{n}$, where $\std(Y) = \sqrt{\int_{\cube} \abs{f(\bx) - \mu}^2 \, \dif \bx}$.  The smoothness required of $f$ is minimal, and the error bound has no curse of dimensionality assuming that $\std(Y)$ does not explode with increasing dimension.  However, the convergence rate is a modest $\Order(n^{-1/2})$, which translates into a computational cost of $\Order(\varepsilon^{-2})$ to satisfy error criterion \eqref{eq:error_crit}.
...
\begin{wrapfigure}{r}{0.45\textwidth}
	...
	\caption{A comparison of the scopes of libraries containing QMC software.  Some libraries with broad scope have limited QMC scope, and vice versa.}
\end{wrapfigure}
...
Each focuses on just certain aspects of QMC, such as the generation of LD sequences, fundamental QMC algorithms, or particular applications.  Some are small libraries, while others have broad scope, but mostly with only a small QMC part.  Whereas SciPy, PyTorch, and TensorFlow are a community efforts, most of the other software packages are the efforts of individual research groups or companies.
...
\subsubsection {Variance/Variation Reduction Through Importance Sampling and Control Variates} [Years 1--3]  The discrepancy in \eqref{eq:KH} measures the difference between the empirical distribution of the sampling sites and the uniform distribution.  In many cases, the original problem is posed as an integral over $\reals^d$, as on the left side of \label{eq:imp}, in which case a variable transformation must be performed to obtain an integral over $[0,1]^d$.  Note that $f_{\bT}(\bx) = g(\bT(\bx)) \lambda(\bT(\bx)) / \abs{(\bT'(\bx))}$, where $\abs{(\bT'(\bx))}$ denotes the Jacobian.  While one may choose $\bT$ such that $\lambda(\bT(\bx)) / \abs{(\bT'(\bx))} = 1$, this is not the only choice and may not be the best choice.  Choosing the transformation $\bT$ is equivalent to performing importance sampling.   

The Koksma-Hlawka inequality, \eqref{eq:KH}, tells us to choose $\bT$ to make the variation, $\norm[\calf]{f_{\bT} - \mu}$, small, which corresopnds to making $f_{\bT}$ as flat as possible.  
...
\begin{theorem}
Let $\epsilon > 0$ be a desired error tolerance. Assume the bias of $f_l$ is bounded as $\sup_{\bx \in [0,1]^d} |f_l(\bx)-f(\bx)| = \mathcal{O}(T_1^{-\alpha l})$ for some $T_1 > 0$. Further assume $C_l$, the cost of a single evaluation of $f_l$, is bounded as $C_l = \mathcal{O}(T_2^{\beta l})$ for some $T_2 > 0$. Suppose the points $\{\bX_i^l\}_{i=1}^{n_l}$ are taken from a low-dispersion QMC sequence (see \cite{niederreiter1992random,yakowitz2000global}). Then, given a total cost budget of $C_\epsilon$ and under mild regularity conditions on $f$, there exists a number of levels $L$ and sample sizes $n_1, \cdots, n_L$ such that $\sup_{\bx \in [0,1]^d} |f(\bx) - \hat{f}_L(\bx)| < \epsilon$ with total cost $C_\epsilon = \mathcal{O}(\epsilon^{-d/\nu})$ if $\alpha d > 2 \beta \nu$.
\end{theorem}
\noindent This theorem shows that, when the bias of the lower-fidelity functions $f_l$ and its evaluation cost are sufficiently bounded, a QMC sampling of design points (optimally allocated over each level) yields the desired error tolerance with total cost bounded by $C_\epsilon = \mathcal{O}(\epsilon^{-d/\nu})$. Under similar conditions, this rate on $C_\epsilon$ can be shown to be faster than existing complexity rates using single-level predictors or Monte Carlo sampling \citep{wendland2004scattered}, thus showing the effectiveness of QMC in jointly reducing approximation error and reducing cost complexity for multi-level approximation.
...
\subsection{Big Data Analytics} [\SM lead]
...
While the existence of a LD subset is promising, one challenge lies in finding such subset efficiently. We will find this subset the following optimization approach. Define the so-called ``data kernel'', defined using the big data $\mathcal{X} = \{\bX_m\}_{m=1}^N$:
\begin{equation}
...
\subsection{Efficient Learning for Expensive Bayesian Models} [\SM lead, \FH, \TT]
...
\subsubsection{Applications} We will also demonstrate the effectiveness of RSPs in a suite of practical Bayesian modeling problems where the posterior is expensive. PI \SM has worked on a range of such problems, including climatology \cite{mak2016regional}, aerospace engineering \cite{mak2018efficient,chang2019kernel,yeh2018common,chang2021reduced}, and nuclear physics \cite{ji2021graphical,everett2021multisystem,everett2021phenomenological,cao2021determining}, and we will explore the effectiveness of RSPs for these applications. 
...
By making QMC methods easily accessible, QMCPy will \emph{introduce new application areas} to the benefits of LD sampling.  This will lead to \emph{LD sampling being incorporated} into the software packages used by practitioners in those disciplines. This includes, e.g., PyMC3 \citep{salvatier2016probabilistic} and PyStan \citep{stan2017pystan}, which are popular Python packages for Bayesian statistical modeling and probabilistic machine learning. The accessibility of state-of-the-art QMC methods will also spur on novel advancements in a diverse range of disciplines. We will focus specifically on three important application areas as proving grounds for QMCPy:
\begin{enumerate}[(i)]
\item \textit{Uncertainty Quantification (UQ)}: UQ is the science of quantifying, characterizing, tracing, and managing uncertainty in computational and physical systems \cite{smartuq, Smi14a}. Since data from such systems are typically expensive to collect, a key focus in UQ is the sampling design for data collection \SMNote{simulation?}, which can greatly benefit from QMC methods. A recent application of QMC for UQ is to compute the expectation of a functional of solution of a partial differential equation with random coefficients \cite{HerSch20a}. This has broad applications in engineering problems, e.g., fluid flow through a porous medium. \SMNote{emphasize win here} QMCPy will implement a typical use case, which may show how to link QMCPy code with a PDE solver from another package. The PI \SM has done extensive work on uncertainty quantification, for rocket engine design \cite{li2017two,li2018uncertainty,chang2019kernel,yeh2018common,mak2018efficient}, neuroscience modeling \cite{wang2020uncertainty}, and active learning \cite{mak2018maximum}, we will explore the performance of LD sampling for these applications.
\item \textit{Machine Learning}: \SMNote{add few sentences before} Image rendering is a visually stunning application of QMC \cite{Keller2013a}.  QMCPy will implement a simple example to showcase the advantage of LD over IID sampling.
\item \textit{Bayesian Modeling}: \SMNote{need? fill in a few sentences.}
\item \textit{Probabilistic Numerics}: \PN assumes that the input function or solution of a mathematical problem is an instance of a random process.  This allows one to apply a probabilistic approach to construct credible intervals for the solution. PI \FH along with his former PhD student \JR developed a fast Bayesian cubature method \cite{RatHic19a} using lattice LD sampling.  Our collaborator \TS is a \PN expert.
