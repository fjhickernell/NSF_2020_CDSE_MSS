...
Our partners include  collaborators \hypertarget{MMlink}{Mike McCourt} (\MM, SigOpt), \hypertarget{AOlink}{Art Owen} (\AO, Stanford University), and \hypertarget{TSlink}{Tim Sullivan} (\TS, Warwick University), \hypertarget{ASlink}{Aleksei Sorokin } (\AS, PhD student, Illinois Tech) as well as other students, alumni, and friends.
...
UQLab \cite{UQLab2014}.  These softwre packages are written in various languages.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Richer LD Sequence Generators} \label{sec:richLD}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Implementing a richer class of LD sequence generators combined with QMCPy's growing set of use cases will allow us to test which LD sequence generators perform better in practice, e.g., Niederreiter vs. Sobol' or LMS vs. NUS. If we detect that certain sequences, or certain scrambling methods substantially outperform in practice, then we will develop a \emph{theoretical description} of when that happens.

\subsection{Theoretical Issues Surrounding Discrepancy and Convergence}
...
An perspective is that we transform our sampling points to mimic the original distribution, $F$, and work with  the error bound derived by \FH in \cite{Hic99a}:
\begin{equation}
...
There is no theory to say under what conditions small  $D(\{\bX_i\}_{i=1}^n)$ implies small  $D(\{\bT(\bX_i)\}_{i=1}^n,F )$.  Indeed our recent study \cite{LiKanHic20a} suggest that this is not always the case.  We will derive conditions under which the transformation of low discrepancy points yields low discrepancy points.  This will involve assumptions on the reproducing kernel $K$ and the distribution $F$.

\FJHNote{Strong law of larg numbers for lattices}


\subsection{Variance/Variation Reduction Through Importance Sampling and Control Variates}
...
\subsection{Multilevel QMC (MLQMC)}
...
\subsection{Posterior Sampling for Complex Bayesian Models}
...
\subsection{PI Yuhan Ding has no prior NSF support to report}
