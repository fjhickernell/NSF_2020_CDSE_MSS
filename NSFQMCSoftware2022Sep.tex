...
Efficient LD sampling is only part of the process of satisfying  error tolerance \eqref{eq:error_crit}.  One also needs reliable bounds on $\abs{\mu - \hmu_n}$  based on the function data, $f(\bX_1), f(\bX_2), \ldots$, that can inform stopping criteria that determine a suitable sample size $n$.  We want to be confident in meeting the desired error tolerance
...
\item Constructions of LD sequences, $\bX_1, \bX_2, \ldots$ require extensive offline computational work.  Furthermore, ideally they require some understanding of which coordinate directions are more significant. We address this limitation in Section \ref{??} 

\item Given a random variable $\bT$ with a known distribution, there are usually multiple possible transformation $\bPsi$ that map to $\bT$ from a standard uniform.  For example, mapping a uniform random $d$-vector to a $d$-dimensional multivariate Gaussian random vector relies on the choice of one of infinitely many possible square roots of the covariance matrix (for $d>1$).  Choosing $\bPsi$ to give an $f$ with small roughness, i.e., small variation, $\norm[\calf]{f - \mu}$ is nontrivial and ideally should be data-based. This equivalent to the problem of adaptive importance sampling. We address this limitation in Section \ref{??}.

\item For many practical problems, the random quantity of interest $Y_\fidparam = f_\fidparam(\bX_\fidparam)$ is parameterized by some vector $\fidparam$, and the true quantity of interest is $\mu_\infty = \lim_{\fidparam \to \fidparam_\infty} \bbE(Y_\fidparam)$.  Here, $\fidparam$ is the \emph{fidelity} parameter and may describe, for example, the mesh size of a solver for a partial differential equation with random coefficients.  Typically, the cost of evaluating $f_\fidparam(\bX_\fidparam)$ depends on $\fidparam$, and so evaluating many high fidelity $f_\fidparam(\bX_\fidparam)$ to approximate $\mu_\infty$ is costly.

Multifidelity methods choose a sequence of fidelity parameters, $\fidparam_1, \fidparam_2, \ldots$ and write the desired answer as a telescoping sum: 
\[
