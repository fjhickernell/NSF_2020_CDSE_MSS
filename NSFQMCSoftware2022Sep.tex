...
\hypertarget{CHlink}{Claude Hall, Jr.} (\CH, African-American PhD student, \hypertarget{IJlink}{Irene Ji} (\IJi, PhD student, Duke U), \hypertarget{JMlink}{John Miller} (\JM, PhD student, Duke U), \hypertarget{KLlink}{Kevin Li} (\KL, PhD student, Duke U), \hypertarget{ASlink}{Aleksei Sorokin} (\AS, PhD student, Illinois Tech),
Illinois Tech), 
...
A problem arising in many applications is the \emph{expectation of a random variable} $Y$ whose distribution is some complicated function, $f$, of a \emph{standard uniform} random vector: \FJHNote{Do we introduce $g$ in this line?}
...
\label{sec:tasks}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We now present a suite of novel methods (with supporting theory and algorithms) which extend LD sampling ideas to the aforementioned complex settings from our motivating applications. These methods provide a useful toolbox for cost-efficient and confident sampling to accelerate discovery in a broad range of modern scientific applications. Figure \ref{fig:workflow} visualizes the workflow for the four proposed tasks introduced next. These tasks address the aforementioned limitations outlined in Section \ref{sec:limit}, and provide a suite of methods which enable cost-efficient and reliable scientific inference given an experimental budget.

\begin{figure}[!t]
\centering
\includegraphics[width=0.7\textwidth]{ProgramsImages/workflow.png}
\caption{Visualizing the workflow for this proposal: given a limited experimental budget, the four proposed tasks in Section \ref{sec:tasks} extend LD sampling to the complex settings motivated by our applications, thus providing a toolbox for cost-efficient and reliable scientific discovery. \cmtS{any comments on this? i suppose we want to add QMCPy as a box within there...}}
\label{fig:workflow}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Bayesian Inference for Expensive Posteriors} [\SM lead, \FH, \IJ, \TT, \JM{}] \label{sec:bayes}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Background \& Preliminary Results}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Bayesian methods have become popular in recent decades, due to their ability to fit complex statistical models via sampling \cite{GelEtal13}. Such methods rely on MCMC sampling for exploration of the posterior distribution $F$, which captures information on model parameters. However, with the rise of expensive scientific simulators and large training datasets, the posterior $F$ can often be \textit{expensive} to evaluate. In our earlier heavy-ion application, each posterior evaluation involves a forward run of expensive computer simulator, which requires thousands of CPU hours. This is compounded by the highly correlated nature of existing MCMC samplers, which reduces the information provided by each sample \citep{link2012thinning}. The use of traditional MCMC samplers for this Bayesian inverse problem can thus be prohibitively \textit{time-consuming} \cite{joseph2015sequential}. For such expensive Bayesian problems, an LD posterior sampling method can provide improved learning of complex models given a computational budget.
...
\end{multline}
where $\nabla$ and $\nabla \cdot$ are the gradient and divergence operators. The key advantage is that the desired integral, $\int K_{\rm ST}(\bt,\bX) \, \dif F(\bt)$, evaluates to 0 for all $\bX$, thus allowing one to generate LD posterior samples via direct discrepancy minimization. With this closed-form $D_K(\{\bX_i\}_{i=1}^n, F)$ with kernel $K=K_{\rm ST}$ (which we call the kernel Stein discrepancy or KSD), \cite{chen2018stein} employ a sequential optimization procedure: the first sample $\bX_1^*$ is taken at the mode of $F$, then subsequent samples $\bX_2^*, \bX_3^*, \cdots$ are obtained by sequentially optimizing the KSD, i.e.:
\begin{equation}
\bX_i^* \leftarrow \argmin_{\bx} D_K(\{\bX_i^*\}_{i=1}^{n-1} \cup \bx, F), \quad K = K_{\rm ST}, \quad n = 2, 3, \cdots.
\label{eq:ksdopt}
\end{equation}
...
However, in view of the aforementioned limitations, Stein points have a key drawback: they are \textit{not} cost-efficient. When the posterior $F$ is expensive, the optimization of the Stein discrepancy for a \textit{single} sample point will require \textit{many} evaluations of $F$ in the form of its score function $\nabla_\bx \log \dif F(\bx)$. The generation of $n$ Stein points can thus require much more than $n$ evaluations of the posterior $F$. For our heavy-ion application, given say a fixed budget of $10^6$ CPU hours, we can afford $10^3$ evaluations of $F$, which translates to only $n \approx 50$ Stein points as posterior samples - this is clearly inadequate for precise posterior exploration.

We thus propose the cost-Efficient Stein Points (ESPs), which make use of a novel extension of Stein points for cost-efficient sampling. The key idea behind ESPs is the construction of a sequence of carefully constructed Gaussian process surrogate models \citep{santner2003design} on the expensive KSD objective functions: this enable the reuse of posterior evaluations for the sequence of optimization problems in \eqref{eq:ksdopt}, and provides. Consider first \eqref{eq:ksdopt} for a given $n$, and suppose the expensive posterior (in the form of its score function) has been at evaluated at $M_{n}$ candidate points $\{\tilde{\theta}_j\}_{j=1}^{M_{n}}$. We then place a 



\begin{figure}[!t]
    \centering
    \includegraphics[width=0.8\textwidth]{ProgramsImages/2d2m_corr_500Evals.png}
    \caption{Caption}
    \label{fig:esps}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Big data is ubiquitous with recent advances in technology and computing power. This is particularly the case in our heavy-ion and UAV applications, where the outputs of the scientific simulators can require exabytes of storage (see \cref{sec:background}). A key challenge is that learning algorithms need to be highly \textit{efficient} and \textit{scalable} to extract useful information from such data for real-time decision-making. One strategy is to iteratively train the model on small batches of the data, typically sampled uniformly at random. This \textit{subsampling} is used to scale up powerful state-of-the-art machine learning algorithms, such as stochastic gradient descent (SGD, \cite{Bot2010}) and stochastic gradient boosting \cite{friedman2002stochastic}.
...
\cmtS{may need to buff up a bit on DEI (I think this was a comment in prior reviews): } As with past projects, in seeking students, we will give preference to underrepresented minorities, women, and students from colleges where research experiences are rare.  As noted in \cref{sec:PreviousFred}, we have had significant success in mentoring students.  Many undergraduates have enrolled in graduate programs.  Five of \FH's fifteen students who earned PhDs are women, three of whom are academics. A  summer 2021 REU student is Black.  Two of \SM's four current PhD students are women. \YDNote{Add something related to SURE program}
