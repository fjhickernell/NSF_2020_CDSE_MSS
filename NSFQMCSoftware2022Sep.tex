...
\cmtS{need student names here?} This project will be led by 
...
A problem arising in many applications is the \emph{expectation of a random variable} $Y$ whose distribution is some known, complicated function, $g$, of a random vector $\bT$ with known distribution. 
...
\subsubsection*{\textup{Limitation 5:}} \hypertarget{LimFiveLink}{\textit{Automatic Variable Transformations}.}  \cmtS{need here?}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Bayesian methods \cite{gelman1995bayesian} rely on MCMC sampling for exploration of the posterior distribution $F$, which captures information on model parameters (see PI \SM's prior work \cite{mak2016regional,huang2022population,mak2021tsec}). However, the posterior $F$ can often be \textit{expensive} to evaluate in many scientific computing problems (see \LimOne). This is compounded by the highly correlated nature of existing MCMC samplers, which reduces the information provided by each sample \citep{link2012thinning}. Traditional MCMC samplers for such problems thus be prohibitively \textit{time-consuming} \cite{joseph2015sequential}, and an LD posterior sampling method can provide improved Bayesian learning given a computational budget.
...
\subsection{QMCPy as a Proving Ground} \label{sec:provingground} \FJHNote{Fred, tighten up}
As mentioned in \cref{sec:lowdiscrep,sec:stopcrit}, \FH, \YD, \SCTC, \AS, \MM, \JR, \PR, and collaborators have created the open source Python QMC library \QMCPy.  \QMCPy has a clear architecture and consistent user interface. It includes \LD sequences,  variable transformations, and stopping rules.  \QMCPy \cite{QMCPy2020a} is hosted on a GitHub repository 
%\cmtS{should we provide link?} it is in the reference
with documentation, a suite of doctests and unit tests, Jupyter notebook demos, and an issues board. With this architecture, \QMCPy will serve as an ideal vessel for bridging our suite of methods (and state-of-the-art \QMC algorithms) to the scientific community.
...
QMCPy will also \emph{stand in the breach} between research code from individual groups and large-scale software packages.  Research groups need to compare their new ideas with the best available.  Those who develop \LD generators need to test them on a variety of use cases and as key components of various QMC algorithms.  Those with new QMC algorithms need to test them with the best generators.  \QMCPy facilitates all of these. We will work to integrate \QMCPy with popular software libraries for scientific computing.  We recently collaborated with Uncertainty Quantification and Model Bridge (UM-Bridge) \cite{umbridge} to make \QMCPy compatible with UM-Bridge.
...
Our team combines senior personnel with diverse backgrounds, career stages, and institutions. The senior personnel---together with our students and collaborators---will have regular meetings to share progress and brainstorm next steps. These will be held both at our own institutions and via video conference between institutions.
